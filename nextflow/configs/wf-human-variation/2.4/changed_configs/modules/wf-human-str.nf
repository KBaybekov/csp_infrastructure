    ...
    script:
        ...
        if [[ -s repeats_subset.bed ]]; then
            straglr-genotype --loci repeats_subset.bed \
                ...
                --min_cluster_size 1 \
                ${xam} ${ref}
            bgzip -c -@ ${task.cpus} ${chr}_tmp.vcf > ${chr}_straglr.vcf.gz
...
process bam_region_filter {
    ...
    script:
        ...
        if [[ -s repeats_subset.bed ]]; then
            samtools view -@ ${task.cpus} -b -h --write-index -o "${xam_meta.sq}.wf_str_regions.bam##idx##${xam_meta.sq}.wf_str_regions.bam.bai" -L repeats_subset.bed ${xam}
        else
            echo "blank subset BED"
        fi
        """
...
process bam_read_filter {
    ...
    shell:
        ...
        samtools view -@ ${task.cpus} --write-index -N reads_to_filter.txt -o !{chr}.wf_str_reads.bam##idx##!{chr}.wf_str_reads.bam.bai !{xam}
        """
}
